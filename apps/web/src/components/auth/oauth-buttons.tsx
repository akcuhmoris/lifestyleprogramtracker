"use client";

import { useState } from "react";
import { Loader2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { createClient } from "@/lib/supabase/client";

type Props = {
  next?: string;
};

export function OAuthButtons({ next }: Props) {
  const [busy, setBusy] = useState<"google" | "apple" | null>(null);

  async function signInWith(provider: "google" | "apple") {
    setBusy(provider);
    const supabase = createClient();
    const redirectTo = `${window.location.origin}/auth/callback?next=${encodeURIComponent(
      next ?? "/"
    )}`;
    const { error } = await supabase.auth.signInWithOAuth({
      provider,
      options: { redirectTo },
    });
    if (error) {
      setBusy(null);
      // Redirect with the error in the URL so the form shows it.
      window.location.href = `/login?error=${encodeURIComponent(error.message)}`;
    }
    // If no error, the browser is about to redirect to the OAuth provider.
  }

  return (
    <div className="space-y-2">
      <div className="relative my-3 flex items-center">
        <span className="flex-1 h-px bg-border-subtle" />
        <span className="px-3 text-[10.5px] uppercase tracking-[0.2em] text-text-dim">
          or
        </span>
        <span className="flex-1 h-px bg-border-subtle" />
      </div>

      <button
        type="button"
        disabled={busy !== null}
        onClick={() => signInWith("google")}
        className={cn(
          "w-full inline-flex items-center justify-center gap-2 rounded-xl border",
          "border-border bg-bg-card px-4 py-2.5 text-[13.5px] font-medium text-text",
          "hover:border-accent/40 hover:bg-bg-hover transition-colors disabled:opacity-60"
        )}
        aria-label="Continue with Google"
      >
        {busy === "google" ? (
          <Loader2 className="h-4 w-4 animate-spin" />
        ) : (
          <GoogleIcon />
        )}
        Continue with Google
      </button>

      <button
        type="button"
        disabled={busy !== null}
        onClick={() => signInWith("apple")}
        className={cn(
          "w-full inline-flex items-center justify-center gap-2 rounded-xl border",
          "border-border bg-bg-card px-4 py-2.5 text-[13.5px] font-medium text-text",
          "hover:border-accent/40 hover:bg-bg-hover transition-colors disabled:opacity-60"
        )}
        aria-label="Continue with Apple"
      >
        {busy === "apple" ? (
          <Loader2 className="h-4 w-4 animate-spin" />
        ) : (
          <AppleIcon />
        )}
        Continue with Apple
      </button>
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg className="h-4 w-4" viewBox="0 0 48 48" aria-hidden>
      <path
        fill="#FFC107"
        d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8c-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4C12.955 4 4 12.955 4 24s8.955 20 20 20s20-8.955 20-20c0-1.341-.138-2.65-.389-3.917"
      />
      <path
        fill="#FF3D00"
        d="m6.306 14.691l6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4C16.318 4 9.656 8.337 6.306 14.691"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238A11.91 11.91 0 0 1 24 36c-5.202 0-9.619-3.317-11.283-7.946l-6.522 5.025C9.505 39.556 16.227 44 24 44"
      />
      <path
        fill="#1976D2"
        d="M43.611 20.083H42V20H24v8h11.303a12.04 12.04 0 0 1-4.087 5.571l.003-.002l6.19 5.238C36.971 39.205 44 34 44 24c0-1.341-.138-2.65-.389-3.917"
      />
    </svg>
  );
}

function AppleIcon() {
  return (
    <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      <path d="M16.365 1.43c0 1.14-.39 2.18-1.17 3.13c-.94 1.13-2.06 1.79-3.27 1.69c-.15-1.11.4-2.27 1.17-3.05c.76-.78 2.05-1.62 3.27-1.77zM20.79 17.21c-.6 1.4-1.32 2.78-2.45 4.18c-1.04 1.27-2.14 1.91-3.31 1.91c-1.12 0-1.92-.59-3.06-.59c-1.18 0-2.04.62-3.12.62c-1.16 0-2.27-.72-3.31-2.04c-2.18-2.78-3.85-7.84-1.61-11.26c1.1-1.68 3.07-2.74 5.13-2.74c1.18 0 2.27.69 3.12.69c.85 0 2.21-.78 3.74-.68c.62.03 2.4.27 3.55 2.01c-3.13 1.7-2.65 6.03.32 7.9z" />
    </svg>
  );
}
