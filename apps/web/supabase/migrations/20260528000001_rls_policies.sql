...
-- ---------- task_details ----------
drop policy if exists task_details_select_own on public.task_details;
create policy task_details_select_own on public.task_details
  for select using (auth.uid() = user_id);
drop policy if exists task_details_insert_own on public.task_details;
create policy task_details_insert_own on public.task_details
  for insert with check (auth.uid() = user_id);
drop policy if exists task_details_update_own on public.task_details;
create policy task_details_update_own on public.task_details
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists task_details_delete_own on public.task_details;
