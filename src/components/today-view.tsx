...
import { dayNumber, formatPretty, TOTAL_DAYS } from "@/lib/date";
...
export function TodayView({
  ...
  const ringValue = dayN <= 0 ? 0 : Math.min(dayN, TOTAL_DAYS) / TOTAL_DAYS;
  ...
  // If journal is checked but content is empty, treat the check toggle as an intent
  // to open the journal modal. If user unchecks via card click, clear the entry.
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10 lg:py-14">
      ...
      <motion.div
        layout
        className="mt-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3"
      >
        {TASKS.map((t, i) => {
      ...
      <AnimatePresence>
        {allDone && (
          ...
          >
            ...
            <div className="mt-2 text-xl font-medium">All 12 tasks done.</div>
            <div className="mt-1 text-sm text-text-muted">
              That&apos;s {dayN} of {TOTAL_DAYS}. Keep it going.
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function Header({
  ...
  const valid = dayN >= 1 && dayN <= TOTAL_DAYS;
        ...
        <h1 className="mt-4 text-4xl font-semibold tracking-tight sm:text-5xl">
          {valid ? (
            <>
              ...
              <span className="text-text-dim"> of {TOTAL_DAYS}</span>
            </>
          ) : dayN < 1 ? (
      ...
      <div className="relative">
        ...
        <div className="relative">
          <ProgressRing
            ...
            size={148}
            stroke={9}
            label={`${Math.max(0, Math.min(dayN, TOTAL_DAYS))}`}
